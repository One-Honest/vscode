window.onload=function(){
    initNav();//1.初始化顶部导航[吸顶效果]

    initGoods();//2.初始化首页的商品
    
    initCategory();//3.初始化首页的分类
    
    initFooter();//初始化底部数据
    
    initToTopBtn();//初始化返回顶部的按钮  
}
function initNav(){//初始化顶部导航
    let oNav=document.querySelector(".nav");
    let navHeight=oNav.offsetHeight;
    let oNav1=document.querySelector(".nav1");
    window.onscroll=function(){
        let offsetY=getPageScroll().y;
        if(offsetY>=navHeight){
            oNav1.style.top="0";
        }else{
            oNav1.style.top="-100px";
        }
    }
}
function initTextRow(){ //2.初始化控制文字行数的代码
    /* //1.获取操作元素
    let oDes=document.querySelector(".mask-des");
    //2.获取文本行高
    let textLineHeight=parseFloat(getComputedStyle(oDes).lineHeight);
    //3.拿到元素的高度
    let oHeight=parseFloat(getComputedStyle(oDes).height);
    //4.判断元素高度是否高于行高的3倍
    if(oHeight>(3*textLineHeight)){
        oDes.style.height=(3*textLineHeight)+"px";
        oDes.style.overflow="hidden";
    } */
    let oDes=document.querySelectorAll(".mask-des");
    for(let i=0;i<oDes.length;i++){
        $clamp(oDes[i], {clamp:4});
    }
}
function initJsonData(){//初始化首页内容的json文件
    let str=`{
        "list": [
          {
            "originW": 750,
      ...
      `;
      return JSON.parse(str);//将json转化为js对象;
}
function waterFall(){//瀑布流流式布局
...
function initGoods(){//初始化首页的商品
  //2.初始化首页数据
  let obj=this.initJsonData();
  this.console.log(obj);
  //3.根据初始化后的数据创建内容
  let oMainIn=document.querySelector(".main-in");
  var html = template('test', obj);
  oMainIn.innerHTML =oMainIn.innerHTML+html;//加上新增的内容,否则.main-in中的内容都被覆盖;
  
  //4.初始化内容中控制文字行数的代码
  initTextRow();
  //5.实现瀑布流排版(解决瀑布流排版混乱有两种方法:1.先加载图片2.创建图片时直接设置宽高)
  waterFall();
}
function initCategoryJsonData(){//初始化分类的json文件
  let str=`{
    "cateList": [
      {
...
function initCategory(){
    //6.初始化banner数据
    let oMainIn=document.querySelector(".main-in");
    let objbanner=initCategoryJsonData();
    var html2 = template('testbanner',objbanner);
...
function initFooter(){
    //7.初始化底部数据
    let objfooter=initFooterJsonData();
    let oFooterIn=document.querySelector(".footer-in");
    var html3 = template('testfooter',objfooter);
